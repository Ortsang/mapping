...
setwd("D:/Drives/Github_Ortsang/mapping")
cbsa_cleaned = fread("data/cbsaDakhipr.csv") 
#movingphysician = fread("moving_physician_bycbsa.csv")
TED2012_collapse <- read_dta("TED/TED2012-collapse.dta")%>%
  mutate(cbsa=as.numeric(CBSA2010))
state = read_excel("data/states.xlsx")
...
cbsa = core_based_statistical_areas(cb = TRUE, year = 2020) %>%
  ...
  left_join(.,TED2012_collapse,by=c("cbsa"))%>%
...
tm_shape(state_outline)+
  ...
  tm_shape(cbsa) + 
  tm_polygons(col = "ADMYR",
              palette = "Blues",
              style = "jenks")


gg <- ggplot() + geom_sf(data = state_outline, 
                     color = "grey", size = 0.1)
...
gg <- gg + scale_fill_continuous(low="thistle2", high="darkred", 
                                  guide="colorbar",na.value="grey") #scale_fill_viridis_c(option = "magma")
